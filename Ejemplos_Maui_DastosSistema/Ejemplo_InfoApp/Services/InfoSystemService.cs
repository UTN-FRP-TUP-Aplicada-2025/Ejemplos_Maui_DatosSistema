...
public class InfoSystemService
{
    ...
    /// </summary>
    public Dictionary<string, string> GetSystemLoad()
    {
        try
        {
#if ANDROID
            ...
            try
            {
                ...
                {
                    var loadAvg = File.ReadAllText("/proc/loadavg").Split(' ');
                    if (loadAvg.Length >= 3)
                    {
                        info.Add("LoadAverage1Min", loadAvg[0]);
                        info.Add("LoadAverage5Min", loadAvg[1]);
                        info.Add("LoadAverage15Min", loadAvg[2]);
                    }
                }
                */
                var procDirs = Directory.GetDirectories("/proc")
                       ...
                       .Take(50);
                var dirList = string.Join(", ", procDirs);
                info.Add("procDirs", dirList);
            }
            catch { }
    ...
    public string GetSystemLoadText()
    {
        var sb = new System.Text.StringBuilder();
        var deviceInfo = GetSystemLoad();
        foreach (var kvp in deviceInfo)
        {
            sb.AppendLine($"{kvp.Key}: {kvp.Value}");
        }
